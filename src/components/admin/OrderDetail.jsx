...
const OrderDetail = () => {
  ...
  const visibleOrders = showAll ? getAllOrder : getAllOrder.slice(0, 8);
            ...
            <div className="flex flex-col items-center justify-center py-10 m-10 text-center bg-p-400 text-white rounded-2xl shadow-lg">
              <div className="">
                ...
                <p className="mt-2 text-sm text-p-100">
                  Looks like there are no orders available right now. Once a
                  customer places an order, <br /> it will appear here.
                </p>

                {/* Action Button (optional) */}
                <button
                  onClick={() => getAllProductFunction()}
                  className="mt-6 rounded-lg bg-white px-5 py-2 text-sm font-medium text-p-500 hover:bg-p-50 transition cursor-pointer"
                >
                  Refresh Orders
                </button>
              </div>
